...
\begin{figure}[H]
	...
	\caption{Beispiel-\textit{Hyperplane}} 
	\label{fig:Hyperplane}
\end{figure}
\todo{Ab hier überarbeiten}
...
Diese Trainingsobjekte werden jeweils einer von zwei Klassen zugeordnet $y_1,\dots,y_n \in \{-1,1\}$.
Wenn die \textit{Hyperplane} die Trainingsobjekte perfekt anhand ihrer Label trennt,
dannt sprcht man von einer \textit{separierenden Hyperplane}. 
...
die richtige Klassifizierung getroffen hat.
...
nächsten Traingsobjekten liegt. 

Der Abstand zwischen der \textit{separierenden Hyperplane} und den nächsten Traingsobjekten
wird als \textit{Margin} bezeichnet. Die \textit{Hyperplane}, deren \textit{Margin} somit am größten ist,
wird auch \textit{Maximal Margin Hyperplane} genannt. 
Die Traingsobjekte, die am nächsten zu der \textit{Hyperplane} liegen und den gleichen Abstand zu dieser haben,
sind die Stützvektoren. Sie 'stützen' die \textit{Maximal Margin Hyperplane}.
Die Stützvektoren sind demnach die einzigen Traingsobjekte, die die Position der \textit{Hyperplane} überhaupt beeinflussen.
...
Je größer dabei die \textit{Margin} der zugrunde liegenden \textit{Hyperplane}, desto erfolgreicher der Klassifizierer.

Je nach Traingsdaten gibt es Szenarien in denen keine \textit{separierende Hyperplane} existiert, die die Daten perfekt
...
und kann übertrainiert werden. Deswegen müssen um eine bessere Klassifizierung und eine höhere Robustheit für einzelne Traingsobjekte
zu ermöglichen auch nicht perfekt \textit{separierende Hyperplanes} in Betracht gezogen werden.
...
wie vielen Traingsobjekten erlaubt werden soll die \textit{Margin} zu durchdringen ohne deren Ausrichtung zu beeinflussen.
...
Durch diese 'weiche' \textit{Margin} werden vereinzelt Testobjekte mit anomalen Charakteristiken der falschen Klasse
...
ist es leichter eine \textit{separierende Hyperplane} zu finden.
...
Traingsobjekte erweitert. Auf Basis der bisherigen \textit{Features} werden so mathematisch neue \textit{Features}, die 
...
\textit{Kernel}-Funktionen muss durch ausprobieren gefunden werden.
...
So kann die \textit{Support Vector Machine} auch Testobjekte verschiedener Klassen unterscheiden.
