...
Hierfür wurde die Programmiersprache \textbf{Python} verwendet, die mit \textbf{Sklearn} bereits eine Bibliothek für die Anwendung 
verschiedener \textit{Machine Learning}-Algorithmen besitzt. Das fertige Programm ist in \ref{lst:test_svm} zu sehen.
Zuerst werden die benötigten Funktionen aus der \textbf{Sklearn}-Bibliothek importiert. Dann wird der 1797-große Datensatz geladen, der die
...
Trainigsdatensatz sowie in einen Testdatensatz aufgeteilt. Mit \textbf{Sklearn} lässt sich dann in zwei Zeilen ein SVM-Objekt erstellen und
...
Funktionen durch die \textbf{Python}-Bibliothek \textbf{Sklearn} bereitgestellt werden. Die \textit{Support Vector Machine} ist damit sehr einfach
...
Die CNN wird ähnlich wie die SVM als ein Konstrukt zum Klassifizieren von Bildern genutzt, jedoch unterscheiden sie sich in der Herangehensweise das Problem zu lösen. Anders als die SVM handelt es sich bei der CNN um keine algorithmische Umsetzung, sondern um eine vordefinierte Menge an Anweisungen, die zur Lösung eines spezifischen Problems führen. Aufgrund der vordefinierten Menge an Anweisungen ist ein CNN auf Probleme beschränkt, welche wir verstehen und lösen können. Dies hat zur Folge, dass kein Ansatz dem anderen überzuordnen ist, sondern die Art des Problems definiert, ob konventionelle Algorithmen oder neuronale Netzwerke besser geeignet sind. \url{https://link.springer.com/chapter/10.1007/978-3-319-45378-1_1}

Um nun den Aufbau eines CNN zuverstehen, betrachten wir zunächst ein normales \textit{Artificial Neural Network} (ANN) oder \textit{Neural Network}. Beide Terminologien werden in der Literatur kommutativ genutzt, um den Unterschied zu den biologischen neuronalen Netzwerken zu verdeutlichen wird fortlaufend in dieser Arbeit ANN als Terminologie genutzt.

Ein ANN ist im Aufbau ähnlich zu einem primitiven biologischen Nervensystem. Es besteht aus einer gewissen Anzahl an Knoten, die miteinander verbunden sind und Informationen an den jeweils anderen Knoten weiterleiten.
In einem ANN sind diese Knoten in verschiedene Schichten eingeordnet und kommunizieren nur Knoten aus der vorherigen und danach kommenden Schicht.
Die Schichten werden in drei Arten unterschieden, der Eingabeschicht, die verstecken Schichten und die Ausgabeschicht.
Die Eingabeschicht nimmt einen mehrdimensionalen Eingabevektor entgegen. Dieser wird in der Eingabeschicht verarbeitet und das Ergebnis wird an die Knoten der nächste versteckte Schicht verteilt. Die danach kommende Schicht verarbeitet die Informationen, die sie bekommen hat, weiter und verteilt diese dann wieder an die nächste versteckte Schicht oder an die Ausgabeschicht.https://arxiv.org/pdf/1511.08458.pdf
